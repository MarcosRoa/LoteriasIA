// ============================================
// UI - Renderização da interface (COM BLOQUEIOS PRO)
// ============================================

import { LOTERIAS, REGRAS_OFICIAIS, VALORES_PIX, CONCURSOS_POR_ANO, cacheDados, cacheDatas } from './loterias.js';
import { mostrarToast, getModoTexto } from './utils.js';
import { abrirModalComprar, iniciarPagamentoPix, comprarPro } from './pagamentos.js';
...
// Garantir que comprarPro esteja disponível globalmente
...
export function atualizarInterfaceUsuario() {
    ...
    if (window.usuarioAtual) {
        ...
        // Badge PRO
        const proBadge = window.usuarioIsPro 
            ? `<span class="pro-badge" title="Plano PRO ativo">⭐ PRO</span>`
            : '';
        
        const proExpiresMsg = window.usuarioIsPro && window.usuarioAtual?.proExpiresAt
            ? `<p class="pro-expires">✨ Válido até ${new Date(window.usuarioAtual.proExpiresAt).toLocaleDateString()}</p>`
            : '';
        ...
        userInfoArea.innerHTML = `
            <div class="user-info">
                ...
                <div class="user-details">
                    <h4>${window.usuarioAtual.nome} ${proBadge}</h4>
                    ${proExpiresMsg}
            ...
            <div class="credits-box" onclick="window.abrirPerfil()">
                ...
                <strong id="creditosDisplay">R$ ${window.creditosUsuario}</strong>
            </div>
...
// Função auxiliar para mascarar configurações (para não PRO)
function mascararConfiguracoes(texto) {
    if (!texto) return '******';
    // Substituir todo o conteúdo por asteriscos
    return texto.split('').map(() => '*').join('');
}

// Exportar função para ser usada no perfil.html
...
// Função para verificar se o usuário é PRO (acessível globalmente)
window.isUserPro = () => window.usuarioIsPro || false;

// ============================================
// FUNÇÃO PARA CARREGAR CSV DA PASTA CORRETA
// ============================================
...
function getPeriodoTexto() {
    if (window.periodoSelecionado === 'all') {
        return `Todos os concursos (${window.dadosAtuais.length} concursos)`;
    }
...
export function renderizarConteudo(loteria) {
    ...
    if (config.temDispersao) controlesExtras += `<div class="dispersao-slider">
        ...
        <input type="range" id="dispersaoSlider" min="${config.dispersaoMin}" max="${config.dispersaoMax}" value="${window.dispersaoAtual}" oninput="window.atualizarDispersao(this.value)">
        <div class="dispersao-valor">Bloquear números recentes: <strong id="dispersaoValor">${window.dispersaoAtual} concursos</strong></div>
    </div>`;
