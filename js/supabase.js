...
export async function salvarTransacaoSupabase(uid, transacao) {
    ...
    try {
        await window.supabaseClient.from('transacoes').insert({ usuario_uid: uid, tipo: transacao.tipo, quantidade: transacao.quantidade, saldo_apos: transacao.saldo, data: new Date() });
...
export async function carregarCreditosSupabase(uid, usuarioAtual) {
    ...
    try {
        const { data, error } = await window.supabaseClient.from('usuarios').select('creditos').eq('uid', uid).maybeSingle();
        if (error) throw error;
        if (!data) {
            await window.supabaseClient.from('usuarios').insert({ uid: uid, nome: usuarioAtual?.nome || '', email: usuarioAtual?.email || '', creditos: 5, created_at: new Date(), updated_at: new Date() });
            return 5;
        }
...
export async function salvarCreditoSupabase(uid, novoSaldo) {
    if (!window.supabaseClient || isAdminUser()) return;
    try {
        const { data: existe } = await window.supabaseClient.from('usuarios').select('uid').eq('uid', uid).maybeSingle();
        if (existe) {
            await window.supabaseClient.from('usuarios').update({ creditos: novoSaldo, updated_at: new Date() }).eq('uid', uid);
        } else {
            await window.supabaseClient.from('usuarios').insert({ uid: uid, creditos: novoSaldo, created_at: new Date(), updated_at: new Date() });
        }
...
export async function salvarHistoricoSupabase(uid, loteria, jogos, filtros, extras, meses = null) {
    if (!window.supabaseClient || isAdminUser()) return;
    try {
        const insertData = { usuario_uid: uid, loteria: loteria, jogos: jogos, filtros: filtros, extras: extras, data: new Date() };
...
export async function carregarHistoricoSupabase(uid) {
    if (!window.supabaseClient || isAdminUser()) return [];
    try {
        const { data } = await window.supabaseClient.from('historico_palpites').select('*').eq('usuario_uid', uid).order('data', { ascending: false }).limit(50);
        return data || [];
    } catch (error) { return []; }
}
...
export async function ativarPro(uid, valorPagamento = 19.90, diasValidade = 15) {
    if (!window.supabaseClient) return false;
    try {
        const { data, error } = await window.supabaseClient.rpc('ativar_pro', { user_uid: uid, valor_pagamento: valorPagamento, dias_validade: diasValidade });
        if (error) throw error;
