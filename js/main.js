...
export function setUsuarioAtual(valor) { window.usuarioAtual = valor; usuarioAtual = valor; }
export function setCreditosUsuario(valor) { window.creditosUsuario = valor; creditosUsuario = valor; }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    onAuthStateChanged(async (user) => { if (user) await processarLogin(user); else atualizarInterfaceUsuario(); });
    console.log('%c✅ Loterias V.6.1 - Modularizado!', 'color: #22c55e; font-size: 14px;');
