...
export async function abrirModalComprar() {
    if (!window.usuarioAtual) {
        const { mostrarModalLogin } = await import('./utils.js');
        mostrarModalLogin();
        return;
    ...
    VALORES_PIX.forEach(v => { 
        const q = v/3; 
        html += `<button onclick="window.iniciarPagamentoPix(${v})" style="background: linear-gradient(135deg,#10b981,#059669);border:none;border-radius:12px;padding:15px;color:white;cursor:pointer;"><strong>R$ ${v}</strong><br><small>${q} jogos</small></button>`; 
    });
    ...
    html += `</div><div class="modal-footer"><button onclick="window.fecharModalComprar()">Fechar</button></div></div></div>`;
...
window.iniciarPagamentoPix = (valor) => {
    import('./pagamentos.js').then(m => {
        m.iniciarPagamentoPix(valor, window.usuarioAtual, window.creditosUsuario, null, null, null);
    });
};
window.fecharModalComprar = fecharModalComprar;